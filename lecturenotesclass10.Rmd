...
37.5 % chance of a blue marble, and a 12.5% chance of a white marble.
...
Quite close to the proportion of the actual probability. This is the law of 
large numbers as the sample size gets larger, the probabilities become more and 
more accurate.
...
'replicate' is for the rolling of the dice so slightly
different than example. 

```{r identify_events}
roll_results <- roll_results %>% 
mutate(option_1 = ((die_1)^2 == die_2), 
        option_2 = abs(die_1 - die_2) == 4) %>%
print()
```

```{r summarize_rolls}
roll_results %>% 
  count(option_1, option_2) %>%
  mutate(probability = n / sum(n))
```

The abosolute value of the difference of the two numbers being 4 is more likely
than the square of dice 1 being equal to dice 2. The former happens 11% of the 
time and the latter happens only ~ 5%. 
